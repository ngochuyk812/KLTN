...
export default function PredictPage() {
  ...
  const generatePDF = useCallback((orderData: any) => {
    ...
    const orderDetails = orderData.order_details.map((detail: any) => [
      detail.product_name,
      detail.quantity,
      `${formatterCurrency.format(detail.price)} `, 
      `${formatterCurrency.format(detail.quantity * detail.price)} `,
    ]);
    ...
    doc.text(
      `Total: ${formatterCurrency.format(orderData.total)}`,
      pageWidth - margin,
          ...
          onChange={(e) => {
            setRows((oldRows) => {
              ...
              if (existingItemIndex !== -1) {
                ...
                newList[existingItemIndex] = newList[0]
                newList[0] = updatedItem;
                return newList;
              }
              return oldRows;
            });
      ...
      <div className={cx("container")}>
        ...
        >
          ...
          <div className={cx("productWrapper")} style={{ flex: 1 }}>
            <Block title="List Product" styles={{display: 'flex', flexDirection: 'column'}}>
              <DataGrid rows={rows} columns={columns} pagination />
              <p >Total Price: {formatterCurrency.format(rows?.reduce((sum, tmp)=> sum + (tmp.quantity * tmp.price), 0))}</p>
            </Block>
